...
| `SideDockWindow` | 🔶 进行中 — Stage A-E + 视觉/功能 parity 完成（`POLARIS_GPU_SIDEDOCK=1`） | D2D 玻璃 slab + 钉住列 + 命中测试/悬停标签 + macOS 放大波 + **运行应用条**（Polaris 居首、绿色运行点、`+N` 溢出、钉住列与运行条间的发光分隔线，放大波横跨两段；按 AUMID/路径排除已钉住应用避免重复）+ **交互**（点击启动钉住/运行项、拖拽重排、拖出删除、从资源管理器拖入固定 `WM_DROPFILES`，经 `ConfigStore.Save` 持久化）+ **与原版对齐**：透明图标底板、柔和径向运行点、启动弹跳、右键菜单（固定/取消固定/关窗，深色玻璃风格 WPF popup，屏幕 DIP 定位居中于图标上方）、悬浮窗口缩略图（复用 `WindowPreviewPopup`，用透明可穿透锚窗口作为 placement target）。**关键 DPI 修复**：`CompositionHost` 须 `_d2d.SetDpi(96×scale)`——D2D 设备上下文 DPI 默认 96 且独立于目标位图 DPI，不设会令全部按 1.0x 绘制（内容偏小、贴左上、命中测试与绘制错位）。每显示器 DPI 感知：布局 DIP，窗口/交换链物理像素；DPI 比例改用 `EnumDisplaySettings`（物理分辨率 ÷ WPF DIP 宽）以避免 `GetDpiForWindow/GetDpiForMonitor` 回落到 96。注意 `_winX/_winY/_winW/_winH` 与 `s.Center` 均为 DIP（窗口物理位置 = `_winX×_dpi`）。剩余：接入真实侧 dock 显隐逻辑/live-sync、翻默认 |
