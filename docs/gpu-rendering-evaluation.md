...
## 10. 独立渲染线程方案（备用计划，待指令再实施）

> 状态：**未实施 / 备用**。本机已 44-53fps 逼近 59Hz 上限，边际收益小；真正大赢家是真实高刷硬件（120/144Hz → ~2-2.4×）。需要时再按本节实施。

### 10.0 结论先行
- 技术可行，是突破本机 ~38-40fps（`CompositionTarget.Rendering`/WPF MediaContext 时钟上限）、吃满高刷屏的**唯一**途径。
- 成本不在"开线程调 Render"，而在**三套线程亲和 API（Win32 HWND / DirectComposition / D3D-D2D）的纪律** + **`Tick` 与 `Render` 间约 30 个共享可变字段的竞态消除**。
- 内存**不降**（+1 线程栈）；降内存的功劳已由 GlassSlab 泄漏修复拿到。
- 建议：**全程藏在 `POLARIS_GPU_RENDERTHREAD=1` 开关后、默认关**，FrameClock 留作回退；真高刷机 A/B 验证稳定后再议默认开启。
...
| **P6** | 验证：VM(~59)、真高刷机(120/144)、内存持平、退出干净、A/B 对比回退 | — | — |
...
### 10.7 优缺点与建议
- **优点**：解锁真实刷新率（VM 稳 59；高刷 120/144，~2-2.4×）；渲染脱离 UI 线程鼠标/消息抢占，节拍更稳更顺。
