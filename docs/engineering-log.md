...
- **多屏：鼠标无法移动到副屏的两侧，被阻挡（低概率、难复现）**：
  ...
  - **修复**：把 clip 矩形的 `Left`/`Right` 也扩到**整个虚拟桌面**（`SM_XVIRTUALSCREEN` / `+SM_CXVIRTUALSCREEN`），与 `Top` 同理——水平方向放行跨屏，只保留当前屏底边（`Bottom=floor`）的钳制；光标移到相邻屏后下一轮（~10ms）`MonitorFromPoint` 重新求得新屏并重算 clip。**单屏时虚拟桌面 X 边界恒等于唯一显示器 X 边界 → `Left`/`Right` 与改前逐位相同 → 零回归**；仅多屏下新增水平跨屏自由。（竖直错位多屏存在可忽略的角落软限制：在 A 屏中央带内时光标暂被钳在 A 的底边，需先水平移到 B 屏才能下探 B 的下沿——非硬阻挡，下一 tick 自愈，远好于原先的硬性水平封锁。）
...
- **悬停预览防误收(透明走廊 + 关闭延迟)+ 放大曲线微调**：解决「鼠标斜向移到预览图时易误收」。①`WindowPreviewPopup` 在预览卡片朝 dock 一侧加一条**透明命中走廊**(宽=ExtraTopLift+27,按 Above/Below/Left/Right 选边),并以 bridge 量反向补偿放置回调使可见卡片位置不变——鼠标一离开图标即落入弹窗命中区(`_pointerInPopup`),不再被判为悬停相邻图标/离开,斜穿不误收;②关闭延迟 220→300ms 作兜底。两个 dock 共用。③放大曲线微调(`DockTuning`):`HoverScale` 1.7→1.8(焦点更大)、`SpreadPush` 0.75→1.2(相邻图标推得更远)。
